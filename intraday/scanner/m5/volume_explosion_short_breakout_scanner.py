...
def is_strong_breakout_candle(breakout_candle,
    ...
    # Body check
    body = breakout_open - breakout_close
    body_pct = body / breakout_candle_range
    if body_pct < body_threshold:
        return False

    # wick check
    lower_wick_pct = (breakout_close - breakout_low) / breakout_candle_range
    if lower_wick_pct > max_wick_ratio:
        return False
...
def is_volume_explosion_short_breakout_detected(breakout_candle):
